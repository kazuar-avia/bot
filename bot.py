...
# ============================================================================
# 🚂 ЛОКАЛЬНИЙ ЛІЧИЛЬНИК РЕСУРСІВ RAILWAY
# Усе знаходиться в bot.py. Жодного Railway API: бот рахує ресурси сам.
# ============================================================================
class LocalRailwayMeter:
    CPU_USD_PER_VCPU_MONTH = 20.0
    RAM_USD_PER_GB_MONTH = 10.0
    ...
    def _save_state(self):
        try:
            ...
            print(f"RESOURCE_METER save error: {e}")
    ...
    def _read_cgroup_cpu():
        ...
        for path in (
            ...
            try:
                return int(Path(path).read_text().strip()) / 1_000_000_000.0
            except Exception:
                pass
        return time.process_time()

    @staticmethod
    def _read_cgroup_ram():
        for path in (
        ...
        for pid in pids:
            try:
                ...
                try:
                    ...
                    cmdline = [x.decode("utf-8", "replace") for x in raw_cmd.split(b"\0") if x]
                except Exception:
                    cmdline = []
    ...
    def _apply(self, prev, cur):
        ...
        cpu_delta = max(0.0, cur["cpu"] - prev["cpu"])
        tx_delta = max(0, cur["tx"] - prev["tx"])
        rx_delta = max(0, cur["rx"] - prev["rx"])
        ram_avg = max(0.0, (prev["ram"] + cur["ram"]) / 2.0)
        volume_avg = max(0.0, (prev["volume"] + cur["volume"]) / 2.0)

        ram_gb_min = ram_avg / 1_000_000_000.0 * dt / 60.0
        volume_gb_min = volume_avg / 1_000_000_000.0 * dt / 60.0

        for bucket in (
            self.state["totals"],
            self.state["daily"].setdefault(cur["wall"].strftime("%Y-%m-%d"), self._empty_usage()),
        ):
            bucket["cpu_seconds"] = float(bucket.get("cpu_seconds", 0.0)) + cpu_delta
            bucket["ram_gb_minutes"] = float(bucket.get("ram_gb_minutes", 0.0)) + ram_gb_min
            bucket["tx_bytes"] = int(bucket.get("tx_bytes", 0)) + tx_delta
        ...
        observed_cpu = sum(proc_cpu.values())
        if observed_cpu > cpu_delta and observed_cpu > 0:
            scale = cpu_delta / observed_cpu
            for label in list(proc_cpu):
                proc_cpu[label] *= scale
        ...
        self.latest = {
            ...
            "tx_bps": tx_delta / dt,
            "rx_bps": rx_delta / dt,
            "volume_bytes": cur["volume"],
        }

        if len(self.state["daily"]) > 90:
            for day in sorted(self.state["daily"])[:-90]:
    ...
    async def _loop(self):
        ...
        while True:
            try:
                ...
                print(f"RESOURCE_METER sample error: {e}")
                await asyncio.sleep(self.interval)
    ...
    def _body_size(data):
        try:
            ...
            if isinstance(data, (list, tuple)):
                return sum(LocalRailwayMeter._body_size(x) for x in data)
            return 0
        except Exception:
            return 0
    ...
    def install_network_hooks(self):
        ...
        if LocalRailwayMeter._installed:
            return
        LocalRailwayMeter._installed = True

        LocalRailwayMeter._orig_request = aiohttp.ClientSession._request
        ...
        if hasattr(aiohttp, "ClientWebSocketResponse"):
            LocalRailwayMeter._orig_ws_str = aiohttp.ClientWebSocketResponse.send_str
            LocalRailwayMeter._orig_ws_bytes = aiohttp.ClientWebSocketResponse.send_bytes
            ...
            aiohttp.ClientWebSocketResponse.send_bytes = wrapped_ws_bytes

    @classmethod
    def _costs(cls, usage):
        cpu = float(usage.get("cpu_seconds", 0.0)) / 60.0 * cls.CPU_USD_PER_VCPU_MONTH / cls.MONTH_MINUTES
        ram = float(usage.get("ram_gb_minutes", 0.0)) * cls.RAM_USD_PER_GB_MONTH / cls.MONTH_MINUTES
        net = float(usage.get("tx_bytes", 0)) / 1_000_000_000.0 * cls.EGRESS_USD_PER_GB
    ...
    async def snapshot(self):
        await self.sample_once()
        async with self._lock:
            return json.loads(json.dumps(self.state)), dict(self.latest)

    async def embed(self, page=0):
...
class LocalRailwayView(discord.ui.View):
    def __init__(self, meter, owner_id):
    ...
    async def _show(self, interaction):
        ...
        await interaction.edit_original_response(
            embed=await self.meter.embed(self.page),
            view=self,
        )
...
def format_flight_for_db(f):
    ...
    cleanFlight = {
        "_id": f.get("_id"),
        "aircraft": {
            "_id": f.get("aircraft", {}).get("_id") if f.get("aircraft") else None,
            "name": correctAircraftName,
            "customName": f.get("aircraft", {}).get("name") if f.get("aircraft") else None,
...
async def move_protected_channel_to_third():
    ...
    if channel is None:
        try:
            ...
            print(f"⚠️ Не знайдено захищений канал {PROTECTED_CHANNEL_ID} для перенесення: {e}")
            return

    try:
        # Виносимо канал з категорії, щоб його позиція рахувалася у загальному списку сервера.
        if getattr(channel, "category", None) is not None:
            await channel.edit(category=None, sync_permissions=False)

        # beginning=True + offset=2 = третє місце у списку (позиції: 0, 1, 2).
        await channel.move(beginning=True, offset=2)
        print(f"↕️ Канал {PROTECTED_CHANNEL_ID} перенесено на 3-тє місце у списку серверу.")
    except discord.Forbidden:
        print("❌ Не вдалося перенести захищений канал: боту потрібне право 'Manage Channels'.")
    except Exception as e:
        print(f"⚠️ Помилка перенесення захищеного каналу: {e}")
    ...
    if interaction.type == discord.InteractionType.component:
        ...
        if custom_id.startswith("gstats_"):
            ...
            if flight_type == "cargo":
                ...
                cargo_actual_pax_flight = int(t.get("payload", {}).get("cargo", 0))
                if cargo_actual_pax_flight == 0: cargo_actual_pax_flight = int(f.get("payload", {}).get("cargo", 0))
                cargo_load_factor = round((cargo_actual_pax_flight / cargo_cap) * 100, 1) if cargo_cap > 0 else 0.0
...
async def on_message(message):
    ...
    if message.content.strip().lower() == "!railway":
        ...
        try:
            view = LocalRailwayView(resource_meter, message.author.id)
            panel = await message.channel.send(
                embed=await resource_meter.embed(0),
                view=view,
            )
            view.message = panel
        except Exception as e:
            print(f"RESOURCE_PANEL_ERROR: {e}")
            await message.channel.send(f"❌ Помилка лічильника: `{str(e)[:1200]}`")
        return
...
async def fetch_airlines_data(session):
    ...
    payload = {"skip": 0, "count": 30, "needle": "", "sort": "stats.recentFlights", "order": -1}
    ...
    return json.dumps(clean_airlines, ensure_ascii=False, indent=4) if clean_airlines else None

async def fetch_awards_data(session):
    ns_headers = {
        "User-Agent": "Mozilla/5.0",
        "Cookie": f"sid={NEWSKY_SID}" if not NEWSKY_SID.startswith("sid=") else NEWSKY_SID,
        "Content-Type": "application/json",
    ...
    payload_awards = {"activeOnly": False, "count": 100, "needle": "", "order": 1, "showClosed": True, "skip": 0, "sort": "title"}
    
    airline_id = "695810be3dc76275ba8befa9"
    async with session.post(f"https://newsky.app/api/airline/{airline_id}/awards", headers=ns_headers, json=payload_awards) as resp:
        if resp.status != 200: return None, None
        awards_data = await resp.json()

    pilots_progress = {}
    payload_pilots = {"count": 100, "needle": "", "order": 1, "skip": 0, "sort": "createdAt"}
    ...
    for award in awards_data.get("results", []):
        ...
        async with session.post(f"https://newsky.app/api/award/{award_id}/pilots", headers=ns_headers, json=payload_pilots) as p_resp:
            ...
            for p_info in pilots_data.get("results", []):
                ...
                if p_id not in pilots_progress:
                    pilots_progress[p_id] = {"pilot_id": p_id, "fullname": p_info.get("fullname", "Unknown"), "awards": []}
                
                completed_legs = len(p_info.get("progress", {}).get("legs", []))
                pilots_progress[p_id]["awards"].append({
                    "award_id": award_id,
                    "award_title": award_title,
                    "status": "completed" if bool(p_info.get("completed")) else "in_progress",
                    "completed_date": p_info.get("completed"),
                    "progress_pct": p_info.get("progressPct", 0),
                    "legs_done": f"{completed_legs}/{total_legs}"
                })
        await asyncio.sleep(0.1)

    awards_str = json.dumps(awards_data, ensure_ascii=False, indent=4)
    pilots_str = json.dumps(list(pilots_progress.values()), ensure_ascii=False, indent=4)
    return awards_str, pilots_str

async def fetch_demand_data(session):
    gh_headers = {"Authorization": f"token {GITHUB_TOKEN}", "Accept": "application/vnd.github.v3.raw"}
    async with session.get(f"https://raw.githubusercontent.com/{GITHUB_REPO}/main/{GITHUB_FILE_PATH}", headers=gh_headers) as raw_resp:
        if raw_resp.status != 200: return None
    ...
    for line in old_text.splitlines():
        ...
        if line.startswith("{") and line.endswith("}"):
            try:
                ...
                if "icao" in data: icaos.append(data["icao"])
            except: continue
    ...
    fresh_data = []
    for icao in icaos:
        async with session.get(f"https://newsky.app/api/airport/{icao}", headers=ns_headers, timeout=10) as ns_resp:
            if ns_resp.status == 200:
                data = await ns_resp.json()
                fresh_data.append(json.dumps(data, ensure_ascii=False, separators=(', ', ': ')))
        await asyncio.sleep(0.1)

    if not fresh_data: return None
